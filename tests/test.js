...
fixture('Basic test').page('http://localhost:3000');
...
test('Handle native confirmation dialog', async t => {
	...
	await t.click(page.submitButton).expect(page.results.innerText).contains('Peter Parker');
});
...
test('Filling a form', async t => {
	...
	await t.wait(500).click(page.submitButton).expect(page.results.innerText).contains('Bruce Wayne');
});
